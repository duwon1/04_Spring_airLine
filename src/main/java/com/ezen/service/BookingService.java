...
public class BookingService {
		...
		try {
			URL url = new URL(
					"https://apis.data.go.kr/1613000/DmstcFlightNvgInfoService/getFlightOpratInfoList?serviceKey"
							...
							+ "BJ%2FS0bkx62BxgaOuuSdnBsIz6%2BpPeUuEw%3D%3D&numOfRows=5000&pageNo=" + page
							+ "&depAirportId=" + FlightInfoService.airPortId.get(depAirportNm) + "&arrAirportId="
			...
			// 연산순서
			// 1) input = br.readLine() : input에 값 대입 -> 얻은 데이터를 하나씩 읽어와서(readLinge메소드)
			// input에 저장하고 그걸 -> sb객체에 저장하기 sb.append(input);
	...
	public int insertReserv(HashMap<String, Object> paramMap1, HashMap<String, Object> paramMap2) {
	...
	public HashMap<String, Object> getInfo(Item item, String flag) {
		// String depAirportNm, String arrAirportNm, Long depPlandTime, Long
		// ReturnPlandTime
		HashMap<String, Object> result = new HashMap<String, Object>();
		...
		if (flag.equals("1")) {
			...
			if (totalCount1 == 0) {
				System.out.println(
						"가는날 일정 :  출발지:" + depAirportNm + " / 도착지:" + arrAirportNm + " / 가는날일자:" + depPlandTime);
			...
			/* 남는좌석 호출 -start 0711 */
			/*
			 * 절차 1. getAirLine메소드로 이코노미/프레스티지(남은좌석) 리스트(A라 칭함)를 불러옴 2. flightItem1(받아온
			 * 노선정보)로 부터 A리스트와 IDENTITY를 비교한다 3. 비교했을 때 같을 경우 A리스트의 남은좌석 정보를
			 * flightItem1(VO)에 매칭해서 넣는다 4. 비교했을 때 다른 경우 flightItem1(VO)에 수동적으로 값을 넣어준다.
			 */
			...
			for (int i = 0; i < flightItems1.size(); i++) {
				...
				for (ReservVO rvo2 : economy_List) {
					if (eitem.getVihicleId() != null && rvo2.getIdentity().equals(tmp3)) {
						...
						switch (aname) {
						case "아시아나항공":
							eitem.setRemain_economy(50);
							break;
						case "에어부산":
							eitem.setRemain_economy(49);
							break;
						case "이스타항공":
							eitem.setRemain_economy(40);
							break;
						case "제주항공":
							eitem.setRemain_economy(45);
							break;
						case "진에어":
							eitem.setRemain_economy(44);
							break;
						case "대한항공":
							eitem.setRemain_economy(51);
							break;
						case "티웨이항공":
							eitem.setRemain_economy(46);
							break;
						case "에어서울":
							eitem.setRemain_economy(47);
							break;
						case "하이에어":
							eitem.setRemain_economy(30);
							break;
						default:
							eitem.setRemain_economy(10);
						}
					}
				}
				...
				for (ReservVO rvo2 : prestige_List) {
					if (eitem.getVihicleId() != null && rvo2.getIdentity().equals(tmp3)) {
						...
						switch (aname) {
						case "아시아나항공":
							eitem.setRemain_prestige(10);
							break;
						case "에어부산":
							eitem.setRemain_prestige(9);
							break;
						case "이스타항공":
							eitem.setRemain_prestige(6);
							break;
						case "제주항공":
							eitem.setRemain_prestige(9);
							break;
						...
						case "티웨이항공":
							...
							break;
						case "에어서울":
							eitem.setRemain_prestige(8);
							break;
						case "하이에어":
							eitem.setRemain_prestige(6);
							break;
						default:
							eitem.setRemain_prestige(5);
						}
					}
				}
			}
			...
			if (totalCount1 == 0) {
				System.out.println(
						"가는날 일정 :  출발지:" + depAirportNm + " / 도착지:" + arrAirportNm + " / 출발일자:" + depPlandTime);
				System.out.println("지정한 조건에 대한 비행편이 존재하지 않습니다.");
				result.put("message", "지정한 조건에 대한 비행편이 존재하지 않습니다.");
				return result;
			}
			FlightInfo depflightInfo = getFlightInfo(depAirportNm, arrAirportNm, depPlandTime, 1);
			ArrayList<Item> flightItems1 = (ArrayList<Item>) depflightInfo.getResponse().getBody().getItems().getItem(); // 최종적으로
																															// 저장되는
																															// 리스트
			...
			if (totalCount2 == 0) {
				System.out.println(
						"돌아오는날 일정 : 출발지:" + arrAirportNm + " / 도착지:" + depAirportNm + " / 출발일자:" + ReturnPlandTime);
				System.out.println("지정한 조건에 대한 비행편이 존재하지 않습니다.");
				result.put("list", "");
				return result;
			}
	 ...
	 * 
	 * int result=2; HashMap<String,Object>paramMap = new HashMap<String, Object>();
	 * paramMap.put("reservNum_return",reservNum_return);
	 * paramMap.put("reservNum_dep",reservNum_dep);
	 * bdao.deletePassenInfo1(paramMap); bdao.deletePassenInfo2(paramMap); return
	 * result; }
	 */
	 ...
	 * System.out.println("서비스 매개변수  :  " + name+" / "+phone);
	 * HashMap<String,Object>paramMap = new HashMap<String, Object>();
	 * paramMap.put("name", name); paramMap.put("phone",phone);
	 * 
	 ...
	 * 
	 * 
	 * if (list.isEmpty()) { result.put("message", "예약내역이 존재하지 않습니다."); } else {
	 * result.put("passenlist", list);
	 * 
	 * }
	 * 
